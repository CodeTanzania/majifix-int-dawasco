...
exports.shouldFetch = function shouldFetch(accountNumber, updatedAt, done) {
  ...
  instance
    .raw(
      `SELECT COUNT(cu.*) as count FROM ${CUSTOMER_TABLE_NAME} AS cu
      LEFT OUTER JOIN ${CURRENT_BILL_TABLE_NAME} as cr
      ON UPPER(LTRIM(RTRIM(cu.CUSTKEY))) = UPPER(LTRIM(RTRIM(cr.CUSTKEY)))
      LEFT OUTER JOIN ${PREVIOUS_BILL_TABLE_NAME} as pr
      ON UPPER(LTRIM(RTRIM(cu.CUSTKEY))) = UPPER(LTRIM(RTRIM(pr.CUSTKEY)))
      WHERE (
      ...
      (
        ...
        OR
        CONVERT(DATETIME, cu.REPLACE(REPLACE(REPLACE(DueDate, ' ', '*^'), '^*', ''), '*^', ' ')) > CONVERT(DATETIME, '${updatedAt}')
    )`
    )
