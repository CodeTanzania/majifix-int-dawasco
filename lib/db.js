...
exports.normalizeAccount = function normalizeAccount(account) {
  ...
  if (!_.isEmpty(jurisdiction)) {
    jurisdiction =
      (_.find(DEFAULT_JURISDICTION_CODES, { code: jurisdiction }) || {}).name;
  }
  ...
  //account address
  const { plot, neighborhood, city } = account;
  _account.address = _.compact([plot, neighborhood, city]).join(', ');
...
exports.getBillHistory = function getBillHistory(accountNumber, done) {
  ...
  instance
    .raw(BILL_HISTORY_QUERY).asCallback(function (error, results) {
      if (!error && results) {
 ...
 * @static
 */
exports.shouldUpdate = function shouldUpdate(accountNumber, updatedAt, done) {
  ...
  instance
    .raw(
      `SELECT COUNT(*) as count FROM ${CUSTOMER_TABLE_NAME}
      WHERE (
        UPPER(LTRIM(RTRIM(CUSTKEY))) = '${identity}'
        OR
        UPPER(LTRIM(RTRIM(METER_REF))) = '${identity}'
      )
      AND
      (
        CONVERT(DATETIME, LAST_PAY_DATE) > CONVERT(DATETIME, '${updatedAt}')
        OR
        CONVERT(DATETIME, REPLACE(REPLACE(REPLACE(DueDate, ' ', '*^'), '^*', ''), '*^', ' ')) > CONVERT(DATETIME, '${updatedAt}')
    )`
    )
...
exports.getAccount = function getAccount(accountNumber, done) {
  ...
  async.waterfall([
    ...
    //normalize results
    let _results =
      ({ account: {}, customer: {}, bills: [], accessors: [] });
    ...
    if (!error) {
      ...
      //set address
      const { plot, neighborhood, jurisdiction, city } = _results.account;
      _results.account.address =
        _.compact([plot, neighborhood, jurisdiction, city]).join(', ');
...
exports.fetchAccount = function fetchAccount(accountNumber, updatedAt, done) {
  ...
  exports
    .shouldUpdate(accountNumber, updatedAt, function (error, shouldUpdate) {
      //error: back off
      if (error) {
        done(error);
      }
