...
exports.getLatestBill = function getLatestBill(done) {

  const QUERY =
    `SELECT TOP 2 * FROM ${PREVIOUS_BILL_TABLE_NAME} ORDER BY IMPORTED_DATE DESC`;

  instance
    .raw(QUERY).asCallback(function (error, results) {
      done(error, results);
    });
